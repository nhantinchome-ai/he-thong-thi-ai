...
// ==========================================
// ĐĂNG KÝ (DÀNH RIÊNG CHO HỌC SINH)
// ==========================================
...
// ==========================================
// ĐĂNG NHẬP (PHÂN LOẠI 3 TẦNG ADMIN - GIÁO VIÊN - HỌC SINH)
// ==========================================
app.post('/api/dang-nhap', async (req, res) => {
    try {
        const { username, password } = req.body;

        // TÀI KHOẢN ADMIN TỐI CAO (CỨNG) - KHÔNG LƯU TRONG DB ĐỂ TRÁNH BỊ HACK
        if (username === "admin_phuc" && password === "phucdeptrai123") {
            return res.status(200).json({ 
                message: "Chào mừng Admin!", 
                data: { id: "ADMIN_ROOT", fullname: "Admin (Xuân Phúc)", role: "admin" } 
            });
        }
        ...
        res.status(200).json({ 
            ...
            data: { id: user._id, fullname: user.fullname, role: user.role, grade: user.grade, studentClass: user.studentClass }
        });
    } catch (error) { res.status(500).json({ message: "Lỗi máy chủ!" }); }
});

// ==========================================
// TẠO TÀI KHOẢN GIÁO VIÊN (CHỈ ADMIN MỚI ĐƯỢC GỌI)
// ==========================================
app.post('/api/tao-giao-vien', async (req, res) => {
    try {
        ...
        // Lưu role là 'teacher'
        const newTeacher = new User({ fullname, username, password: hashedPassword, role: 'teacher' });
...
app.post('/api/tao-de-thi', async (req, res) => {
    try {
        ...
        const promptYeuCau = `Hãy đóng vai giáo viên ra đề: "${topic}". Yêu cầu: 20 câu (trắc nghiệm + tự luận). Chống gian lận. Trả về đúng JSON: [{"type": "trắc nghiệm", "questionText": "?", "options": ["A","B","C","D"], "correctAnswer": "A"}, {"type": "tự luận", "questionText": "?"}]`;
...
app.listen(PORT, () => console.log(`=> May chu 3 TANG dang chay tai cong: ${PORT}`));
